#include<iostream>

class Veggie {
protected:
  char *name;
  int calories;
public:
  Veggie() : calories(0), name(0) { }

  Veggie(int c) : calories(c), name(0) { }

  Veggie(const char *n, int c) : calories(c) {
    int len = 0;
    while (n[len++]) { }
    name = new char[len];
    for (int i = 0; i < len; ++i) {
      name[i] = n[i];
    } 
  }

  Veggie(Veggie &v) : calories(v.calories) {
    int len = 0;
    while(v.name[len++]) { }
    name = new char[len];
    for(int i = 0; i < len; ++i) {
      name[i] = v.name[i];
    }
  }

  Veggie &operator=(Veggie &v) {
    calories = v.calories;
    int len = 0;
    while(v.name[len++]) { }
    name = new char[len];
    for(int i = 0; i < len; ++i) {
      name[i] = v.name[i];
    }

    return *this;
  }

  void print() { 
    if (name) {
      std::cerr << name << " calories " << calories << std::endl;
    } else {
      std::cerr << calories << std::endl;
    }
  }

  ~Veggie() { 
    if (name) {
      std::cerr << "deallocating " << name << std::endl;
    }
    delete [] name;  // does not do anything if name is zero
  }
};

class Store : public Veggie {
  float price;
  float storage_temperature;

public:
  Store(const char *n, int c, float p, float s) : Veggie(n, c), price(p), storage_temperature(s) { }
};

class Animation : public Veggie {
  int scale;
  int orientation;

public:
  Animation(const char *n, int c, int s, int o) : Veggie(n, c), scale(s), orientation(o) { }
};

int main() { 
  Veggie a("kale", 42);
  Veggie b("brocc", 33);
  Veggie c(10);
  Veggie w(a);
  Veggie x = b;
  Veggie* d[5];
  d[0] = &a;
  d[1] = &b;
  d[2] = &c;
  d[3] = &w;
  d[4] = &x;
  Veggie **e = d;
  e[2]->print();  // first line of output;  where are the others?
}